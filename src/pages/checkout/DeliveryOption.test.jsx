...
describe('DeliveryOptions component',() => {
    ...
    it('renders delivery options correctly',()=> {
        ...
        expect(
            within(deliveryOptionElems[0]).getbyTestId('delivery-option-input').checked).toBe(false);
       ...
       expect(deliveryOptionElems[1]).toHaveTextContent('$4.99 - Shipping');
        ...
        expect(
            within(deliveryOptionElems[1]).getbyTestId('delivery-option-input').checked).toBe(true);
        ...
        expect(
            within(deliveryOptionElems[2]).getbyTestId('delivery-option-input').checked).toBe(false);

    });

    it('updates the dellivery Option', async () => {
        render (
            <DeliveryOptions 
            cartItem={cartItem}
            deliveryOptions = {deliveryOptions}
