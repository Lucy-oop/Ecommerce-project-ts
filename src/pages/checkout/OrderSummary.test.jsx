...
it('renders cart items correctly',()=> {
    ...
    const cartItemContainers = screen.getByTestId('cart-item-container');
    expect (cartItemContainers.length).toBe(2);
    
    expect(
        within(cartItemContainers[0]).getByTestId('cart-item-image')).toHaveAttribute('src', 'images/products/asthletic-cotton-socks-6-pairs.jpg');
    expect(
...
it('does not render anything if cart is empty',() => {
    render (
        <MemoryRouter>
            <OrderSummary 
            cart={[]} deliveryOptions={deliveryOptions}
             loadCart={loadCart}
            />
        </MemoryRouter>
    );
    expect(screen.queryAllByAltText('cart-item-container').length).toBe(0);
})
